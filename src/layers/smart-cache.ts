...
export class SmartCache<T> extends EventEmitter {
  ...
  private handleFileWatcherEvent(filePath: string, eventType: string, filename: string | null, zoneName: string): void {
  ...
  private async evictIfNecessary(newEntrySize: number): Promise<void> {
    ...
    if (currentSize.entries >= this.config.maxSize || 
  ...
  private cleanup(): void {
    ...
    for (const [path, watcher] of this.fileWatchers.entries()) {
      ...
      if (!hasReferences && now - watcher.lastEvent > 300000) { // 5 minutes inactive
