...
import { EnhancedGrep, EnhancedGlob, EnhancedLS, EnhancedSearchTools, createEnhancedSearchTools } from '../enhanced-search-tools.js';
  ...
  describe('Error Handling', () => {
    it('should handle non-existent paths gracefully', async () => {
      ...
      try {
        ...
        const lsResults = await searchTools.ls.list({
          path: nonExistentPath
        }).catch(() => ({ entries: [], metadata: { totalEntries: 0, searchTime: 0, errors: ['Path not found'], path: nonExistentPath } }));
