...
export class ConfidenceCalculator {
    ...
    private consistencyScore(examples: Example[]): number {
        ...
        let score = 0;
        if (consistency >= 0.8) score = 0.2;      // Very consistent
        else if (consistency >= 0.6) score = 0.15; // Moderately consistent  
    ...
    private isInflection(token1: string, token2: string): boolean {
        ...
        // Plural/singular
        if ((token1.endsWith('s') && token1.slice(0, -1) === token2) ||
            (token2.endsWith('s') && token2.slice(0, -1) === token1)) {
            return true;
        }
        
        // Past tense variations
        if ((token1.endsWith('ed') && token1.slice(0, -2) === token2) ||
            (token2.endsWith('ed') && token2.slice(0, -2) === token1)) {
            return true;
        }
        
        // Progressive tense variations
        if ((token1.endsWith('ing') && token1.slice(0, -3) === token2) ||
