...
export class PatternLearner extends EventEmitter {
    ...
    async learnFromRename(oldName: string, newName: string, context: RenameContext): Promise<LearningResult> {
        if (oldName === newName) {
    ...
    private createPatternCandidate(oldName: string, newName: string, context: RenameContext): PatternCandidate {
    ...
    async findApplicablePatterns(identifier: string): Promise<Pattern[]> {
        ...
        for (const pattern of this.patterns.values()) {
            if (this.patternMatches(pattern.from, identifier) && pattern.confidence >= this.confidenceThreshold) {
    ...
    private transformTokens(tokens: string[], fromPattern: TokenPattern[], toPattern: TokenPattern[]): string[] | null {
    ...
    private selectBestExamples(examples: Example[], count: number): Example[] {
        return examples.sort((a, b) => b.confidence - a.confidence).slice(0, count);
    }
    ...
    private calculatePredictionConfidence(
        ...
        if (recentUse) {
            const recencyBoost = Math.max(0, 1 - (Date.now() - recentUse.timestamp.getTime()) / (1000 * 60 * 60)); // 1 hour
