    ...
    try {
      return await withMcpErrorHandling('MCPAdapter', `tool_${name}`, async () => {
        ...
        if (!validTools.includes(name)) {
          return handleAdapterError(new Error(`Unknown tool: ${name}. Valid tools: ${validTools.join(', ')}`), 'mcp');
        }
      ...
      try {
        ...
        let prioritized = Array.isArray(result.data)
          ? result.data.slice().sort((a: any, b: any) => {
              const prioKind = (k: string) => (k === 'class' ? 4 : k === 'function' ? 3 : k === 'interface' ? 2 : k === 'variable' ? 1 : 0);
              const toBase = (u: string) => {
                try { const p = new URL(u).pathname; return p.split('/').pop() || p; } catch { return u.split('/').pop() || u; }
              };
          ...
          if (!likelyTop) {
            ...
            if (Array.isArray(prioritized) && prioritized.length) {
              ...
              for (const def of prioritized.slice(0, 200)) {
                try {
                  const filePath = (() => { try { return new URL(def.uri).pathname; } catch { return def.uri.replace(/^file:\/\//, ''); } })();
          ...
          try {
            ...
            for (let i = 0; i < lines.length; i++) {
              ...
              if (declRe.test(line)) {
                ...
                return {
                  uri: `file://${p}`,
                  range: { start: { line: i, character: col }, end: { line: i, character: col + symbol.length } },
                  kind: /class\s+/.test(line) ? 'class' : /function\s+/.test(line) ? 'function' : /interface\s+/.test(line) ? 'interface' : 'variable',
                  name: symbol,
