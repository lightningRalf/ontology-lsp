 ...
 * for both stdio and SSE transports.
 */

import { fileLogger, mcpLogger } from './file-logger.js';
import { globalErrorHandler, ErrorContext } from './error-handler.js';
...
export class ConnectionManager {
  ...
  getMetrics(): ConnectionMetrics {
    ...
    const currentUptime = this.state === 'connected' && this.metrics.lastConnected 
  ...
  private async attemptReconnection(): Promise<void> {
    ...
    const delay = this.config.reconnectBackoff * Math.pow(2, this.metrics.reconnectAttempts - 1);
