...
export class CodeAnalyzer {
    ...
    async findDefinition(request: FindDefinitionRequest): Promise<FindDefinitionResult> {
        ...
        // Async-first: delegate to streaming/async implementation and return
        ...
        try {
            ...
            if (this.config.layers.layer1.enabled) {
                ...
                try {
                    ...
                    if (!shouldEscalateToLayer2) {
                        console.debug(
                            `Skipping Layer 2 escalation: Layer 1 found ${fastResults.length} high-confidence definitions`
                        );
                    }
                } catch (error) {
                    layerTimes.layer1 = Date.now() - layer1Start;
            ...
            const shouldRunLayer2 =
                this.config.layers.layer2.enabled &&
            ...
            if (shouldRunLayer2) {
                ...
                try {
                    ...
                    if (candidateFiles.size < 5) {
                        const highConfidenceDefinitions = definitions.filter(
                            (d) => d.confidence > 0.8 && d.layer === 'layer1'
                        );
                    ...
                    if (process.env.DEBUG) {
                        const optimizationRatio =
                            allPotentialFiles.size > 0 ? candidateFiles.size / allPotentialFiles.size : 1;
