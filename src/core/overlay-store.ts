...
export class OverlayStore {
  ...
  async cleanup(maxKeep = 10, maxAgeMs = 3 * 24 * 60 * 60 * 1000): Promise<void> {
    ...
    for (const s of toDelete) {
      ...
      try { await fsp.rm(path.join(snapsRoot, s.id), { recursive: true, force: true }); } catch {}
    }
  }
  ...
  private async ensureMaterialized(snapshotId: string): Promise<string | null> {
    ...
    if (!exists) {
      ...
      // Prefer rsync for speed; fallback to cp -a
      if (this.which('rsync')) {
        spawnSync('bash', ['-lc', `rsync -a --delete --exclude .git --exclude node_modules --exclude .ontology --exclude dist ./ ${dir}/`], { stdio: 'pipe' });
      } else {
        spawnSync('bash', ['-lc', `cp -a . ${dir}`], { stdio: 'pipe' });
      }
    }
    ...
    if (snap.diffs.length > 0) {
      ...
      if (this.which('git')) {
        const applied = spawnSync('bash', ['-lc', `git -C ${JSON.stringify(dir)} apply --whitespace=nowarn overlay.diff`], { stdio: 'pipe' });
  ...
  async runChecks(snapshotId: string, commands: string[], timeoutSec = 120): Promise<{ ok: boolean; output: string; elapsedMs: number }>{
