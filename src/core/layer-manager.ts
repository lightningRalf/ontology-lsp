...
export class LayerManager implements ILayerManager {
  ...
  async dispose(): Promise<void> {
    ...
    // Dispose all layers
    const disposePromises = Array.from(this.layers.values())
      .map(layer => layer.dispose().catch(error => {
  ...
  isHealthy(): boolean {
    ...
    const healthyLayers = Array.from(this.layers.values())
      .filter(layer => layer.isHealthy()).length;
  ...
  getPerformanceReport(): PerformanceReport {
    ...
    const weightedLatency = allMetrics.reduce((sum, m) => {
  ...
  /**
   * Execute multiple layers in sequence with cascade fallback
   */
  async executeCascade<T>(
    layerNames: string[],
    operation: string,
    requestMetadata: RequestMetadata,
    executor: (layer: Layer, layerName: string) => Promise<T | null>
  ): Promise<T> {
    let lastError: Error | null = null;
    
    for (const layerName of layerNames) {
      try {
        const result = await this.executeWithLayer(
          layerName,
          operation,
          requestMetadata,
          (layer) => executor(layer, layerName)
        );
        ...
        // Log cascade fallback
        this.eventBus.emit('layer-manager:cascade-fallback', {
          fromLayer: layerName,
          operation,
        ...
        // Continue to next layer unless it's the last one
    ...
    throw lastError || new Error(`All layers failed for operation: ${operation}`);
  }
