...
export async function advancedExample() {
  ...
  const { analyzer, layerManager, sharedServices } = 
    await AnalyzerFactory.createAnalyzer(customConfig);
  ...
  sharedServices.eventBus.on('performance-validator:report', (report: any) => {
    ...
    report.layerResults.forEach((result: any) => {
      if (result.layer !== 'total') {
        console.log(`  ${result.layer}: ${result.current.toFixed(1)}ms avg (target: ${result.target}ms) - ${result.status}`);
      }
    });
...
export async function workspaceExample(workspacePath: string) {
  ...
  try {
    ...
    completions.data.forEach(completion => {
      console.log(`  ${completion.label} (${completion.kind}) - confidence: ${(completion.confidence * 100).toFixed(0)}%`);
    });
