...
import { TeamKnowledgeSystem, TeamMember, SharedPattern } from '../team-knowledge.js';
import { SharedServices } from '../../core/services/index.js';
import { EventBusService } from '../../core/services/event-bus-service.js';
import { Pattern, PatternCategory } from '../../types/core.js';
...
describe('TeamKnowledgeSystem', () => {
  ...
  describe('Initialization', () => {
    ...
    test('should load existing team data', async () => {
      ...
      `, ['test-user', 'Test User', 'developer', '["typescript","react"]', 
  ...
  describe('Pattern Sharing', () => {
    ...
    test('should share pattern successfully', async () => {
      ...
      const patternId = await teamKnowledge.sharePattern(
        testPattern,
        contributorId,
        documentation,
        'team'
    ...
    test('should sort recommendations by success rate', async () => {
      ...
      if (recommendations.length > 1) {
        for (let i = 0; i < recommendations.length - 1; i++) {
          expect(recommendations[i].metrics.successRate)
            .toBeGreaterThanOrEqual(recommendations[i + 1].metrics.successRate);
        }
      }
    });
  ...
  describe('Performance', () => {
    ...
    test('should handle many patterns efficiently', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        patternPromises.push(teamKnowledge.sharePattern(pattern, contributorId, {
  ...
  describe('Error Handling', () => {
    ...
    test('should handle missing pattern during validation', async () => {
      ...
      await expect(teamKnowledge.validatePattern('non-existent', validatorId, {
        status: 'approve',
        score: 4.0,
        feedback: 'Good pattern',
        criteria: { correctness: 4, usefulness: 4, clarity: 4, completeness: 4 }
