...
import { Pattern, Suggestion } from '../types/core.js';
import { PatternLearner } from '../patterns/pattern-learner.js';
import { FeedbackLoopSystem, FeedbackEvent, FeedbackStats, LearningInsight } from './feedback-loop.js';
import { CodeEvolutionTracker, EvolutionEvent, EvolutionPattern, ArchitecturalTrend, EvolutionReport } from './evolution-tracker.js';
import { TeamKnowledgeSystem, TeamMember, SharedPattern, TeamInsight, KnowledgeGraph } from './team-knowledge.js';
...
export class LearningOrchestrator {
  ...
  async learn(context: LearningContext, data: any): Promise<LearningResult> {
    ...
    try {
      ...
      if (totalTime > this.config.performanceTargets.maxLearningTime) {
        console.warn(`Learning operation exceeded target time: ${totalTime}ms > ${this.config.performanceTargets.maxLearningTime}ms`);
      }
  ...
  private async initializeLearningComponents(): Promise<void> {
    ...
    if (this.config.enabledComponents.evolutionTracking) {
      this.evolutionTracker = new CodeEvolutionTracker(
        this.sharedServices,
        this.eventBus,
        {
          minOccurrences: 3,
    ...
    if (this.config.enabledComponents.teamKnowledge) {
      this.teamKnowledge = new TeamKnowledgeSystem(
        this.sharedServices,
        this.eventBus,
        {
          minValidators: 2,
  ...
  private async executePatternLearning(context: LearningContext, data: any): Promise<any> {
    ...
    if (data.prediction) {
      return await this.patternLearner.predictNextRename(
        data.prediction.identifier,
        data.prediction.context
      );
    }
  ...
  private triggerEventDrivenPipelines(eventName: string, data: any): void {
    for (const [pipelineId, pipeline] of this.pipelines) {
      if (pipeline.trigger === 'event_driven' && 
