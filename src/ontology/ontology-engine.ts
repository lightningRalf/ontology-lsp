...
export class OntologyEngine extends EventEmitter {
    ...
    private async calculateConceptSimilarity(identifier: string, concept: Concept): Promise<number> {
        ...
        const canonicalSimilarity = await this.similarityCalculator.calculate(identifier, concept.canonicalName);
    ...
    getStatistics(): {
        ...
        const topConcepts = conceptStats.sort((a, b) => b.representations - a.representations).slice(0, 10);
