/**
 * New HTTP Server using HTTP Adapter - Thin wrapper around unified core
 * Target: ~100 lines (down from 700+)
 * 
...
export class SimpleHTTPServer {
  ...
  private config: SimpleHTTPServerConfig;
  private serverConfig: ServerConfig;
  private server: any = null;

  constructor(config: SimpleHTTPServerConfig = {}) {
...
export let httpServer: SimpleHTTPServer | null = null;

/**
 * Create and start HTTP server
 */
export async function createHTTPServer(config?: SimpleHTTPServerConfig): Promise<SimpleHTTPServer> {
  if (httpServer) {
    await httpServer.stop();
  }

  httpServer = new SimpleHTTPServer(config);
  return httpServer;
}
