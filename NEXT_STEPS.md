...
- Edge-case parity: normalize error shapes across LSP/CLI/HTTP/MCP (target ≥80% consistency)
- Learning determinism: ensure ≥1 pattern learned in E2E by seeding via small dev pipeline
- Threshold calibration: relax or make environment-aware E2E consistency thresholds for local fixture
- MCP robustness: consider tiny debounce to avoid repeated init in quick retries

### 0.35 Graph Expand Hardening
- Strengthen callers/callees detection under fallback
- Track error-note incidence in `/metrics`

### 0.355 MCP Adapter Mapping Consistency
- Audit completion item mapping parity across adapters
- Map to LSP numeric kinds consistently; ensure HTTP tool and MCP return aligned fields
...
- Tests: seed pipelines, assert DB rows exist, schedule parsed
...
### 0.4 CI & Spec Compliance
- CI: add jobs to run `tsc -p tsconfig.build.json`
...
- LSP spec audit: ensure no non-spec fields in LSP responses; document custom executeCommand payloads
- Env gating: document Postgres/Triple tests (run only when `PG_URL`/`TRIPLESTORE_URL` set)
