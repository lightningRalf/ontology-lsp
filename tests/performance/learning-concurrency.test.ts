...
import { FeedbackEvent, EvolutionEvent, CoreConfig, EventBus } from "../../src/learning/types.js";
...
const createCtx = async (): Promise<Ctx> => {
  ...
  const feedbackLoop = new FeedbackLoopSystem(sharedServices, eventBus, { minFeedbacksToLearn: 10, weakPatternThreshold: 0.5, strongPatternThreshold: 0.8 });
  const evolutionTracker = new CodeEvolutionTracker(sharedServices, eventBus, { maxHistorySize: 1000, analysisDepth: 5, patternDetectionThreshold: 0.7 });
  const teamKnowledge = new TeamKnowledgeSystem(sharedServices, eventBus, { validationThreshold: 0.8, sharingEnabled: true, conflictResolutionStrategy: 'vote' });
  const learningOrchestrator = new LearningOrchestrator(sharedServices, eventBus, { enabledComponents: { patternLearning: true, feedbackLoop: true, evolutionTracking: true, teamKnowledge: true } });
...
const makeEvolution = (): EvolutionEvent => ({
  ...
  timestamp: new Date(),
  before: { path: "file:///test/example.ts", content: "function oldImplementation() { return 'old'; }", signature: "oldImplementation()" },
  after: { path: "file:///test/example.ts", content: "function newImplementation() { return 'new'; }", signature: "newImplementation()" },
    ...
    for (let i = 0; i < concurrentOperations; i++) {
      operations.push(
        ctx.learningOrchestrator.learn(
          { requestId: `concurrent-${i}`, operation: 'comprehensive_analysis', timestamp: new Date(), metadata: { index: i } },
