...
describe('HTTP /api/v1/pipelines (register)', () => {
  ...
  test('400 on missing required fields', async () => {
    const res = await fetch(`${base}/api/v1/pipelines`, { method: 'POST', headers: { 'content-type':'application/json' }, body: JSON.stringify({}) });
  ...
  test('200 on register and retrievable via GET /pipelines/{id}', async () => {
    ...
    const res = await fetch(`${base}/api/v1/pipelines`, { method: 'POST', headers: { 'content-type':'application/json' }, body: JSON.stringify(req) });
