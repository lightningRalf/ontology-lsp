...
describe('Snapshot apply/checks (partial materialize)', () => {
  ...
  test('patch_checks_in_snapshot builds http with partial snapshot', async () => {
    ...
    // unwrap
    let json: any;
    try { json = typeof result?.content?.[0]?.text === 'string' ? JSON.parse(result.content[0].text) : result; } catch { json = result; }
