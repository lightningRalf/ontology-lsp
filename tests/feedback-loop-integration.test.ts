...
import { FeedbackLoopSystem, FeedbackEvent } from '../src/learning/feedback-loop.js';
...
describe('FeedbackLoopSystem Integration', () => {
  ...
  describe('Learning from Corrections', () => {
    ...
    test('should handle similar corrections (pattern refinement)', async () => {
      ...
      const corrections = feedbackLoop.getRecentFeedback(10)
        .filter(f => f.type === 'modify');
  ...
  describe('Statistics and Analytics', () => {
    ...
    test('should handle empty statistics gracefully', async () => {
      ...
      const emptyConfig = { ...mockConfig, layers: { ...mockConfig.layers, layer3: { ...mockConfig.layers.layer3, dbPath: emptyDbPath } } };
  ...
  describe('Performance Requirements', () => {
    ...
    test('should handle high volume feedback efficiently', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        promises.push(feedbackLoop.recordFeedback({
          type: i % 2 === 0 ? 'accept' : 'reject',
          suggestionId: `bulk-${i}`,
