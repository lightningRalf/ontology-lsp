...
describe('Tool-First Gate (HTTP tools/call)', () => {
...
test('rename_safely (runChecks=false) yields snapshot + diff', async () => {
    ...
    const totalEdits = plan?.summary?.totalEdits ?? Object.values(plan?.changes || {}).reduce((a: number, v: any) => a + (Array.isArray(v) ? v.length : 0), 0);
