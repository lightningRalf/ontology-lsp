...
import { ensureTestDirectories, cleanupTestDirectories, testPaths, createTestFile, testFileContents } from './test-helpers';
...
describe('Layer 2 AST cap boundaries (L2_MAX_PARSE_FILES)', () => {
  ...
  beforeAll(() => {
    ...
    for (let i = 0; i < 25; i++) {
      createTestFile(path.join('.test-workspace', 'l2cap-boundaries', `file${i}.ts`), testFileContents.typescript);
    }
  });

  afterAll(() => {
    // Restore env and cleanup
    if (prevEnv === undefined) delete process.env.L2_MAX_PARSE_FILES; else process.env.L2_MAX_PARSE_FILES = prevEnv;
