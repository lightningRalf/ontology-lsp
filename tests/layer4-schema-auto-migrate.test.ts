...
describe('L4 Schema Auto-Migration', () => {
    ...
    test('schema is auto-created in constructor (L4_AUTO_MIGRATE=1 default)', async () => {
        ...
        expect(stats.totalRepresentations).toBe(0);
    ...
    test('initialize() is safe to call after constructor auto-migration', async () => {
        ...
        await storage.saveConcept(concept1);
        ...
        await storage.saveConcept(concept2);
    ...
    test('schema includes all expected tables', async () => {
        ...
        expect(tables).toContain('representations');
        expect(tables).toContain('relations');
        expect(tables).toContain('evolution_history');
        expect(tables).toContain('concept_metadata');
    ...
    test('schema includes expected indices', async () => {
        ...
        expect(indices).toContain('idx_representations_name');
        expect(indices).toContain('idx_representations_concept_id');
        expect(indices).toContain('idx_relations_from');
        expect(indices).toContain('idx_relations_to');
