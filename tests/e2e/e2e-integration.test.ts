    ...
    describe('Comprehensive Cross-Protocol Consistency', () => {
        test('should validate comprehensive protocol consistency', async () => {
            ...
            for (const repo of context.testRepositories) {
                ...
                console.log(
                    `   Consistent Cases: ${consistencyPercentage.toFixed(1)}% ≥ 70% ${consistencyPercentage >= 70 ? '✅' : '❌'}`
                );
                console.log(
                    `   Average Similarity: ${similarityPercentage.toFixed(1)}% ≥ 75% ${similarityPercentage >= 75 ? '✅' : '❌'}`
                );
