...
export class LearningEffectivenessValidator {
  ...
  async startLearningSession(): Promise<string> {
    ...
    console.log(`📊 Initial state: ${initialState.totalPatterns} patterns, avg confidence: ${this.calculateAverageConfidence(initialState)}`)
  ...
  async endLearningSession(): Promise<LearningEffectivenessReport> {
    ...
    console.log(`📈 Final state: ${finalState.totalPatterns} patterns (+${finalState.totalPatterns - this.currentSession.initialState.totalPatterns})`)
  ...
  logLearningReport(report: LearningEffectivenessReport) {
    ...
    if (report.patterns.mostFrequent.length > 0) {
      ...
      report.patterns.mostFrequent.slice(0, 3).forEach((pattern, index) => {
        console.log(`   ${index + 1}. ${pattern.pattern} (${pattern.frequency}x, confidence: ${(pattern.confidence * 100).toFixed(0)}%)`)
      })
      console.log(``)
    }
    ...
    if (report.patterns.emergingPatterns.length > 0) {
      ...
      report.patterns.emergingPatterns.forEach((pattern, index) => {
        console.log(`   ${index + 1}. ${pattern.pattern} (${pattern.operations} recent ops, confidence: ${(pattern.confidence * 100).toFixed(0)}%)`)
      })
      console.log(``)
    }
