...
export class CrossProtocolValidator {
  ...
  private async executeTestCaseAcrossProtocols(testCase: ProtocolTestCase): Promise<ProtocolResult[]> {
    ...
    for (const protocol of protocols) {
      ...
      try {
        result = await this.executeOperationOnProtocol(
          protocol.name,
          protocol.adapter,
          testCase
        )
  ...
  private generateAnalysis(
    ...
    if (consistency.allSucceeded) {
      ...
      if (failed.length > 0) {
        const errorSummary = failed.map(f => `${f.protocol}: ${f.error?.split('.')[0] || 'Unknown error'}`).join(', ')
  ...
  private generateConsistencyReport(validationResults: ConsistencyValidationReport["testCases"]): ConsistencyValidationReport {
    const consistentCases = validationResults.filter(vr => vr.consistency.similarResults && vr.consistency.allSucceeded)
    const inconsistentCases = validationResults.filter(vr => !vr.consistency.similarResults || !vr.consistency.allSucceeded)

    // Calculate average similarity
    const similarities = validationResults.map(vr => vr.consistency.similarity)
    const averageSimilarity = similarities.length > 0 ? similarities.reduce((sum, s) => sum + s, 0) / similarities.length : 0

    // Calculate average performance variance
    const variances = validationResults.map(vr => vr.consistency.performanceVariance)
    const averagePerformanceVariance = variances.length > 0 ? variances.reduce((sum, v) => sum + v, 0) / variances.length : 0
    ...
    for (const protocol of protocols) {
      const protocolResults = validationResults.flatMap(vr => vr.results.filter(r => r.protocol === protocol))
    ...
    validationResults.forEach(vr => {
      vr.results.filter(r => !r.success).forEach(r => {
  ...
  private logConsistencyReport(report: ConsistencyValidationReport) {
    ...
    console.log(`   Consistent Cases: ${report.summary.consistentCases} (${Math.round(report.summary.consistentCases / report.summary.totalTestCases * 100)}%)`)
    console.log(`   Inconsistent Cases: ${report.summary.inconsistentCases} (${Math.round(report.summary.inconsistentCases / report.summary.totalTestCases * 100)}%)`)
