...
export class PerformanceBenchmark {
  ...
  async runBenchmarks(
    ...
    for (let i = 0; i < operationCount; i++) {
      ...
      if ((i + 1) % 10 === 0) {
        ...
        console.log(`    📈 Recent 10 operations: avg=${Math.round(recentAvg)}ms, success=${Math.round(recentSuccessRate * 100)}%`)
      }
    }
  ...
  private generateOperationSummary(operationType: string): OperationSummary {
    const operationResults = this.results.filter(r => 
      r.metadata?.type === operationType
    )
  ...
  private logReport(report: BenchmarkReport) {
    ...
    Object.entries(report.summary).forEach(([operation, summary]) => {
      if (summary.count > 0) {
        ...
        if (summary.errors.length > 0) {
          console.log(`     Errors: ${summary.errors.slice(0, 2).join(', ')}${summary.errors.length > 2 ? '...' : ''}`)
        }
      }
    })
    console.log(``)
    ...
    console.log(`   Average Time: ${Math.round(report.averageTime)}ms / ${avgTarget}ms ${report.averageTime <= avgTarget ? '✅' : '❌'}`)
    console.log(`   P95 Time: ${Math.round(report.p95)}ms / ${p95Target}ms ${report.p95 <= p95Target ? '✅' : '❌'}`)
    console.log(`   Memory Growth: ${Math.round(report.memoryGrowth)}MB / ${memoryTarget}MB ${report.memoryGrowth <= memoryTarget ? '✅' : '❌'}`)
    console.log(`   Success Rate: ${Math.round(report.successRate * 100)}% / 80% ${report.successRate >= 0.8 ? '✅' : '❌'}`)
  }
...
export class BenchmarkComparator {
  static async compareReports(
    ...
    if (avgChange > 10) {
      regressions.push(`Average response time increased by ${Math.round(avgChange)}% (${Math.round(baselineReport.averageTime)}ms → ${Math.round(currentReport.averageTime)}ms)`)
    } else if (avgChange < -10) {
      improvements.push(`Average response time decreased by ${Math.round(Math.abs(avgChange))}% (${Math.round(baselineReport.averageTime)}ms → ${Math.round(currentReport.averageTime)}ms)`)
    }

    // Compare P95 times
    const p95Change = ((currentReport.p95 - baselineReport.p95) / baselineReport.p95) * 100
    if (p95Change > 15) {
      regressions.push(`P95 response time increased by ${Math.round(p95Change)}% (${Math.round(baselineReport.p95)}ms → ${Math.round(currentReport.p95)}ms)`)
    } else if (p95Change < -15) {
      improvements.push(`P95 response time decreased by ${Math.round(Math.abs(p95Change))}% (${Math.round(baselineReport.p95)}ms → ${Math.round(currentReport.p95)}ms)`)
    }

    // Compare success rates
    const successRateChange = currentReport.successRate - baselineReport.successRate
    if (successRateChange < -0.05) {
      regressions.push(`Success rate decreased by ${Math.round(Math.abs(successRateChange) * 100)}% (${Math.round(baselineReport.successRate * 100)}% → ${Math.round(currentReport.successRate * 100)}%)`)
    } else if (successRateChange > 0.05) {
      improvements.push(`Success rate increased by ${Math.round(successRateChange * 100)}% (${Math.round(baselineReport.successRate * 100)}% → ${Math.round(currentReport.successRate * 100)}%)`)
    }

    // Compare memory usage
    const memoryChange = ((currentReport.memoryGrowth - baselineReport.memoryGrowth) / baselineReport.memoryGrowth) * 100
    if (memoryChange > 20) {
      regressions.push(`Memory growth increased by ${Math.round(memoryChange)}% (${Math.round(baselineReport.memoryGrowth)}MB → ${Math.round(currentReport.memoryGrowth)}MB)`)
    } else if (memoryChange < -20) {
      improvements.push(`Memory growth decreased by ${Math.round(Math.abs(memoryChange))}% (${Math.round(baselineReport.memoryGrowth)}MB → ${Math.round(currentReport.memoryGrowth)}MB)`)
    }
