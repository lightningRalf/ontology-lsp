...
describe('MCP apply_after_checks add-new-file (unified)', () => {
  ...
  test('creates a brand-new file and reverts it', async () => {
    ...
    try { await fs.rm(targetAbs, { force: true }); } catch {}
    const existsBefore = await fs.readFile(targetAbs, 'utf8').then(() => true).catch(() => false);
    ...
    if (out.applied) {
      ...
      const existsAfterRevert = await fs.readFile(targetAbs, 'utf8').then(() => true).catch(() => false);
