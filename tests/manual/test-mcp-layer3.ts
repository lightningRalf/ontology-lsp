...
async function testMCPLayer3() {
    ...
    try {
        ...
        if (result.content && Array.isArray(result.content)) {
            const conceptualResults = result.content.filter(
                (item: any) => item.text && item.text.includes('"source": "conceptual"')
            );
            ...
            if (conceptualResults.length > 0) {
                ...
                const validUris = conceptualResults.filter(
                    (item: any) => item.text && !item.text.includes('file://unknown')
                );
