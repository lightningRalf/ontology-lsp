...
import { RelationType, type Concept, type SymbolRepresentation } from '../src/types/core';
...
(hasPg ? describe : describe.skip)('Layer 4: Cross-adapter parity (SQLite → Postgres)', () => {
  test('export from SQLite and import into Postgres; compare k-hop at depths 1 and 2', async () => {
    ...
    const relSql1 = sqlite.getRelatedConcepts(A.id, 1).map((r) => r.concept.canonicalName).sort();
    const relSql2 = sqlite.getRelatedConcepts(A.id, 2).map((r) => r.concept.canonicalName).sort();
    ...
    const relPg1 = pg.getRelatedConcepts(A.id, 1).map((r) => r.concept.canonicalName).sort();
    const relPg2 = pg.getRelatedConcepts(A.id, 2).map((r) => r.concept.canonicalName).sort();
...
describe('Layer 4: Cross-adapter parity (SQLite → TripleStore)', () => {
  test('export from SQLite and import into TripleStore; compare k-hop at depths 1 and 2', async () => {
    ...
    const relT1 = tstore.getRelatedConcepts(A.id, 1).map((r) => r.concept.canonicalName).sort();
    const relT2 = tstore.getRelatedConcepts(A.id, 2).map((r) => r.concept.canonicalName).sort();
