// @bun
// src/layers/enhanced-search-tools.ts
...
// src/layers/smart-cache.ts
import * as fs from "fs/promises";
import * as path2 from "path";
import { watch } from "fs";
import { EventEmitter as EventEmitter2 } from "events";

class SmartCache extends EventEmitter2 {
  config;
  cache = new Map;
  accessOrder = new Map;
    ...
    try {
      ...
      if (entry.filePath && entry.fileModTime) {
        ...
        if (zone?.checkFileModTime) {
          try {
            const stat2 = await fs.stat(entry.filePath);
            const currentModTime = stat2.mtime.getTime();
      ...
      this.updateHitRate();
      this.emit("cache:hit", { key, zone: entry.filePath ? this.getZoneForPath(entry.filePath)?.name : "unknown" });
      return entry.data;
    } catch (error) {
      ...
      if (options.filePath) {
        ...
        if (zone) {
          ...
          try {
            ...
            fileModTime = stat2.mtime.getTime();
            fileSize = stat2.size;
  ...
  async invalidateByPath(filePath) {
    const normalizedPath = path2.resolve(filePath);
    const keysToInvalidate = [];
    for (const [key, entry] of this.cache.entries()) {
      if (entry.filePath === normalizedPath || entry.dependencies && entry.dependencies.includes(normalizedPath)) {
  ...
  async invalidateByZone(zoneName) {
    const keysToInvalidate = [];
    for (const [key, entry] of this.cache.entries()) {
      if (entry.filePath) {
        const zone = this.getZoneForPath(entry.filePath);
  ...
  isHealthy() {
    ...
    return !this.disposed && size.entries < this.config.maxSize && size.memoryBytes < this.config.maxMemory && size.watchers < this.config.maxWatchers && (stats.hits + stats.misses === 0 || stats.hitRate > 0.1);
  }
  async dispose() {
    if (this.disposed)
      return;
  ...
  matchesPattern(filePath, pattern) {
    ...
    if (pattern.includes("**")) {
      ...
      const basePath = path2.resolve(basePattern || ".");
      return normalizedPath.startsWith(basePath);
    }
    const regexPattern = pattern.replace(/\./g, "\\.").replace(/\*\*/g, ".*").replace(/\*/g, "[^/]*").replace(/\?/g, ".");
    const regex = new RegExp(`^${regexPattern}$`);
    return regex.test(normalizedPath) || regex.test(path2.basename(normalizedPath));
  }
  async setupFileWatcher(filePath, zoneName) {
  ...
  handleFileWatcherEvent(filePath, eventType, filename, zoneName) {
  ...
  async setupGitWatcher() {
    ...
    if (!gitDir)
      return;
    try {
      const gitHeadPath = path2.join(gitDir, "HEAD");
  ...
  async findGitDirectory() {
    ...
    while (dir !== root) {
      ...
      try {
        const stat2 = await fs.stat(gitDir);
  ...
  async checkDependencies(dependencies) {
    for (const dep of dependencies) {
      try {
        const stat2 = await fs.stat(dep);
        for (const [key, entry] of this.cache.entries()) {
          if (entry.dependencies && entry.dependencies.includes(dep)) {
            if (stat2.mtime.getTime() > entry.timestamp) {
              return true;
            }
          }
        }
  ...
  async evictIfNecessary(newEntrySize) {
    ...
    if (currentSize.entries >= this.config.maxSize || currentSize.memoryBytes + newEntrySize > this.config.maxMemory) {
      const accessEntries = Array.from(this.accessOrder.entries()).sort((a, b) => a[1] - b[1]);
  ...
  cleanup() {
    ...
    for (const [path3, watcher] of this.fileWatchers.entries()) {
      let hasReferences = false;
      for (const entry of this.cache.values()) {
        if (entry.filePath === path3) {
          hasReferences = true;
          break;
        }
      }
      if (!hasReferences && now - watcher.lastEvent > 300000) {
        watchersToRemove.push(path3);
      }
    }
    for (const path3 of watchersToRemove) {
      const watcher = this.fileWatchers.get(path3);
      if (watcher) {
        try {
          watcher.watcher.close();
        } catch (error) {
          this.emit("cache:warning", { message: "Error closing unused watcher", path: path3, error });
        }
        this.fileWatchers.delete(path3);
      }
    }
...
function createSmartCache(overrides = {}) {
  const config = {
    maxSize: 1000,
...
// src/layers/enhanced-search-tools.ts
var __dirname = "/home/lightningralf/programming/ontology-lsp/src/layers";
...
class EnhancedGrep {
  ...
  constructor(config = {
    ...
    if (config.enableSmartCache) {
      this.smartCache = createSmartCache(config.cacheConfig);
  ...
  async searchWithRipgrep(params) {
  ...
  buildRipgrepArgs(params) {
    ...
    if (params.contextAround)
      args.push(`-C ${params.contextAround}`);
  ...
  parseRipgrepOutput(output, params) {
    if (!output.trim())
      return [];
    const results = [];
...
`);
    for (const line of lines) {
      try {
        ...
        const match2 = this.parsePlainTextLine(line, params);
        if (match2)
          results.push(match2);
      }
    }
  ...
  async getFilesToSearch(searchPath, params) {
    const files = [];
    const stat3 = await fs2.stat(searchPath);
    if (stat3.isFile()) {
      return [searchPath];
    }
    if (stat3.isDirectory()) {
      const entries = await fs2.readdir(searchPath, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.name.startsWith(".") && !params.path?.includes(entry.name)) {
          continue;
        }
        const fullPath = path3.join(searchPath, entry.name);
        if (entry.isFile()) {
  ...
  async searchFile(filePath, params) {
    const results = [];
    try {
      ...
      for await (const line of rl) {
        ...
        if (this.matchesPattern(line, params.pattern, params.caseInsensitive)) {
          ...
          if (contextSize > 0) {
            match2.context = {
              before: contextBuffer.slice(0, -1),
              after: []
            };
          }
          results.push(match2);
  ...
  matchesPattern(text, pattern, caseInsensitive = false) {
    try {
  ...
  extractMatch(text, pattern, caseInsensitive = false) {
    try {
      const flags = caseInsensitive ? "i" : "";
      const regex = new RegExp(pattern, flags);
  ...
  matchesFileType(filePath, type) {
    ...
    return typeMap[type]?.includes(ext2) || false;
  }
  parsePlainTextLine(line, params) {
...
class EnhancedGlob {
  ...
  async executeGlob(params) {
    ...
    const files = await glob(params.pattern, options);
    ...
    for (const file of files) {
      try {
        ...
        } else if (stat3.isDirectory() && params.includeDirectories) {
  ...
  async sortByModificationTime(files) {
    const stats = await Promise.allSettled(files.map(async (file) => ({
      file,
      mtime: (await fs2.stat(file)).mtime
    })));
    const validStats = stats.filter((s) => s.status === "fulfilled").map((s) => s.value).sort((a, b) => b.mtime.getTime() - a.mtime.getTime());
...
class EnhancedLS {
  ...
  async listDirectory(dirPath, params, entries, errors, depth) {
    if (params.maxDepth && depth >= params.maxDepth) {
      return;
    }
    try {
      const dirEntries = await fs2.readdir(dirPath, { withFileTypes: true });
      for (const dirent of dirEntries) {
        ...
        const fullPath = path3.join(dirPath, dirent.name);
        try {
  ...
  async createLSEntry(dirent, fullPath, params) {
    const entry = {
      name: dirent.name,
      path: fullPath,
      type: this.getEntryType(dirent),
      extension: path3.extname(dirent.name).toLowerCase()
    };
    if (params.includeMetadata) {
      try {
        ...
        try {
          await fs2.access(fullPath, fsSync.constants.R_OK);
          entry.isReadable = true;
        } catch {
          entry.isReadable = false;
        }
        try {
          await fs2.access(fullPath, fsSync.constants.W_OK);
          entry.isWritable = true;
        } catch {
          entry.isWritable = false;
        }
        try {
          await fs2.access(fullPath, fsSync.constants.X_OK);
  ...
  formatPermissions(mode) {
    const permissions = [];
    permissions.push(mode & 256 ? "r" : "-");
    permissions.push(mode & 128 ? "w" : "-");
    permissions.push(mode & 64 ? "x" : "-");
    permissions.push(mode & 32 ? "r" : "-");
    permissions.push(mode & 16 ? "w" : "-");
    permissions.push(mode & 8 ? "x" : "-");
    permissions.push(mode & 4 ? "r" : "-");
    permissions.push(mode & 2 ? "w" : "-");
    permissions.push(mode & 1 ? "x" : "-");
  ...
  sortEntries(entries, sortBy, sortOrder = "asc") {
    entries.sort((a, b) => {
      let comparison = 0;
      switch (sortBy) {
        case "name":
...
class EnhancedSearchTools {
  ...
  ls;
  constructor(config) {
    this.grep = new EnhancedGrep(config?.grep);
    this.glob = new EnhancedGlob(config?.glob);
    this.ls = new EnhancedLS(config?.ls);
  }
  getMetrics() {
    return {
      ...
      combined: {
        totalSearches: this.grep.getMetrics().searchCount + this.glob.getMetrics().searchCount + this.ls.getMetrics().searchCount,
        totalTime: this.grep.getMetrics().totalTime + this.glob.getMetrics().totalTime + this.ls.getMetrics().totalTime,
        totalErrors: this.grep.getMetrics().errors + this.glob.getMetrics().errors + this.ls.getMetrics().errors
      }
    };
  }
  async healthCheck() {
...
var enhancedSearchTools = new EnhancedSearchTools;
export {
  enhancedSearchTools,
  EnhancedSearchTools,
  EnhancedLS,
